...
> **ここから先は自由に拡張してみよう。** ツールの追加、SystemPrompt のチューニング、UI の改善など、REPL を実際に使い物にするための拡張に取り組む。拡張の方向性とロードマップは `docs/guide/advanced-topics.md` の「Stage EX: REPL 実用化ロードマップ」を参照。
