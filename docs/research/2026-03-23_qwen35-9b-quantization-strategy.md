...
## GPU WS: Qwen3.5-35B-A3B の量子化選定

### 環境

- GPU: RTX PRO 4500 Blackwell, 32GB GDDR7, 896 GB/s
- Driver: 595.45.04, CUDA 13.2

### 量子化バリアント（Unsloth GGUF）

| 量子化 | Disk Size | 実 VRAM 消費 | KV キャッシュ余力 |
|--------|-----------|-------------|-----------------|
| Q4_K_S | 20.7 GB | — | — |
| **Q4_K_M** | **22.0 GB** | **28,056 MiB (~27.4GB)**（実機確認） | **~4.5 GB** |
| Q6_K | 28.9 GB | — | ⚠️ ほぼゼロと推定 |

### 採用: Q4_K_M

- Q6_K は disk 28.9GB → 実 VRAM は 33GB+ と推定され、32GB に収まらない可能性が高い
- Q4_K_M は disk 22.0GB → 実 VRAM 27.4GB。残り ~4.5GB で `-c 8192` に十分
- Q4_K_S は Q4_K_M の下位互換（attention 層の精度が低い）。1.3GB の節約に見合わない
...
→ GPU WS では 35B-A3B (Q4_K_M) 一択。9B は Mac ローカル専用。

---

## 次のアクション

### 自宅 Mac (Qwen3.5-9B 8-bit)
...
4. 8-bit で問題があれば OptiQ-4bit → uniform 4-bit の順でフォールバック

### GPU WS (Qwen3.5-35B-A3B)
5. **Q6_K vs Q4_K_M 精度比較検証**: Stage 1-2 のテストケースを両量子化で実行し、構造化出力・tool calling の精度差を定量化する。明確な差が出る場合、VRAM 余力（KV キャッシュ圧迫）と精度のトレードオフを再評価する。Q6_K は mmproj なし起動で VRAM 消費を削減して検証。
6. 大学 WS (35B-A3B) vs 自宅 Mac (9B 8-bit) の同一テストケース比較で品質差を定量化
