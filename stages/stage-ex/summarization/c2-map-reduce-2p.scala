// Stage EX: C2 — Map-Reduce 2並列×3バッチ
// Run: scala-cli run stages/stage-ex/summarization/c2-map-reduce-2p.scala
...
@main def run(): Unit = {
  val conditionId = "c2"
  val parallelism = 2
  val batches = List(List(0, 1), List(2, 3), List(4, 5))

  println(s"=== Condition C2: Map-Reduce 2-parallel × 3 batches ===")
  ...
  // Thinking 記録 + パート保存
  val thinkingLog = new StringBuilder("# C2 Thinking Log\n\n")
  ...
  val metrics = buildMetricsMapReduce("C2", s"${parallelism}-parallel x ${batches.size}-batches",
  ...
  printFinalSummary("C2", totalWallMs, totalPrompt, totalCompletion)
}
